...
int COBDevice::SendCmd(uint16_t cmd, void *cmdbuf, uint16_t cmd_len, void *replybuf, uint16_t reply_len) {
...
#ifdef WIN32
  res = xnUSBSendControl(m_usbHandle, XN_USB_CONTROL_TYPE_VENDOR, 0x00, 0x0000, 0x0000, (XnUChar *)obuf, cmd_len + sizeof(*chdr), 5000);
#endif

#ifdef LINUX
  res = libusb_control_transfer(handle, LIBUSB_ENDPOINT_OUT | LIBUSB_REQUEST_TYPE_VENDOR, 0x00, 0x0000, 0x0000, (unsigned char *)obuf,
                                cmd_len + sizeof(*chdr), 1000);
#endif
  ...
  do {
#ifdef WIN32
    xnUSBReceiveControl(m_usbHandle, XN_USB_CONTROL_TYPE_VENDOR, 0x00, 0x0000, 0x0000, (XnUChar *)ibuf, 0x200, &actual_len, 5000);
#endif

#ifdef LINUX
    actual_len =
        libusb_control_transfer(handle, LIBUSB_ENDPOINT_IN | LIBUSB_REQUEST_TYPE_VENDOR, 0x00, 0x0000, 0x0000, (unsigned char *)ibuf, 0x200, 1000);
#endif
